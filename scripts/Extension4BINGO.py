...
def nsamples(params_CS=None,params_path=None, path=None, listdir="21cm", nseed_used=False):
    try:
        ...
        pass
    names  = np.asarray(os.listdir(path))
    if listdir in ["21cm","foregrounds","mixmatrix"]:
        if params_CS['restart'] or (params_CS['n_realizations']<names.size):
            names  = np.random.choice(names, size = params_CS['n_realizations'], replace=False)
            nseed0 = extracting_seed_from_filenames(vectornames=names)
            ...
            notdone = np.setdiff1d(nseed,done)    
            names   = extracting_filename_from_seed(vectornames=names, vectorseeds=notdone, path=path)[0]
            nseed0  = extracting_seed_from_filenames(vectornames=names)
        return names, nseed0
        ...
        names   = extracting_filename_from_seed(vectornames=names, vectorseeds=notdone, path=path)[0]
        nseed  = extracting_seed_from_filenames(vectornames=names)
