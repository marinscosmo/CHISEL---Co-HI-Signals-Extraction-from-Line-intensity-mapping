...
#General variables
n_realizations = config.getint(    "General","n_realizations")
...
try:
	...
	L = None
needlet   = config.get(         "Dictionary","needlet")
Bneed     = config.getfloat(    "Dictionary","Bneed")
p         = config.getfloat(    "Dictionary","p")
lmax_need = config.getfloat(    "Dictionary","lmax_need")
try:
	fneed  = json.loads(config.get(    "Dictionary", "fneed"))
	for i in range(len(fneed)): 
		fneed[i]=float(fneed[i])
except:
	raise NameError
...
parser.add_argument('--n_realizations', action = 'store', dest = 'n_realizations', default = n_realizations,  help = 'Number of realizations.')
...
parser.add_argument('--needlet'       , action = 'store', dest = 'needlet'       , default =needlet ,         help = 'If you can choose either mexican(=gaussian) or standard')
parser.add_argument('--Bneed'         , action = 'store', dest = 'Bneed'         , default =Bneed   ,         help = 'Filter is defined by function b(l/B**j), where j=frequency')
parser.add_argument('--p'             , action = 'store', dest = 'p'             , default =p       ,         help = 'This value is only used for mexican needlet')
#parser.add_argument('--fneed'         , action = 'store', dest = 'fneed'         , default =fneed   ,         help = 'Number of frequencies will be the number of scales (wavelet maps). Center of i-band will be approached given by l_center_i = B**freq_i (p=0)')
parser.add_argument('--lmax_need'     , action = 'store', dest = 'lmax_need'     , default =lmax_need,        help = 'Max value used for multipole l.')
...
n_realizations = int(arguments.n_realizations)
...
params_CS   = pd.Series({"n_realizations":n_realizations, "method":method,
...
names  = np.random.choice(names, size = params_CS['n_realizations'], replace=False)
nseed0 = cs.extracting_seed_from_filenames(vectornames=names)
...
names, nseed  = cs.extracting_filename_from_seed(vectornames=names, vectorseeds=nseed0, path=params_path.dir_noise)
...
names, nseed  = cs.extracting_filename_from_seed(vectornames=names, vectorseeds=nseed0, path=params_path.dir_prior)
...
names, nseed  = cs.extracting_filename_from_seed(vectornames=names, vectorseeds=nseed0, path=params_path.dir_pure)
